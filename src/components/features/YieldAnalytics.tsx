...
export function YieldAnalytics({ positions }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display font-bold text-xl text-white">Yield Analytics</h2>
        <p className="text-sm text-[#9CA3AF] mt-1">Performance breakdown across vaults</p>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          <p className="label-section mb-1">Total Yield</p>
          <p className="data-lg text-[#10B981]">+{formatUsd(totalYield)}</p>
        </Card>
        <Card>
          <p className="label-section mb-1">Total IL</p>
          <p className="data-lg text-[#F59E0B]">{formatUsd(totalIL)}</p>
        </Card>
        <Card>
          <p className="label-section mb-1">Net Return</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Bar Chart */}
        <Card>
          <p className="label-section mb-4">Yield vs Impermanent Loss</p>
          <div className="h-64">
        ...
        {/* Pie Chart */}
        <Card>
          <p className="label-section mb-4">Yield Distribution</p>
      ...
      {/* Breakdown Table */}
      <Card>
        <p className="label-section mb-3">Detailed Breakdown</p>
        {yieldBreakdown.map((y, i) => (
          <div key={y.vaultAddress} className="flex items-center justify-between py-3 border-b border-white/8 last:border-0">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
              <span className="text-sm text-[#9CA3AF]">{y.vaultName}</span>
            </div>
